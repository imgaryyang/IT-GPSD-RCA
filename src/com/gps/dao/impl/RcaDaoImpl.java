...
public class RcaDaoImpl implements RcaDao {
    ...
    public Set<Rca> getRcaListByMonthAndYearForReports(int month, int year) {
        Set<Rca> rcas = null;
        StringBuilder queryString = new StringBuilder();
        queryString.append("from Rca r LEFT JOIN FETCH r.rcaActions ra LEFT JOIN FETCH r.contract c LEFT JOIN FETCH c.sessionAcls acl ");
        queryString.append("where r.month = :month and r.year = :year ");
        queryString.append("and acl.role IN ('reportAdmin') ");
